...
| 2025-12-23 | 1.2 | Refinamiento técnico completado con descomposición en tareas y estrategia de testing | esteban.colorado (Developer) |
...
El arquitecto identificó 9 hitos de implementación con patrón Strategy + Factory para creación de contactos. La implementación reutiliza `ContactUtil.createNewContactDummy()` existente y extiende `CommonTransformer` para soportar PersonDummy_Ext y CompanyDummy_Ext. El flujo de carga masiva (2 fases: Azure + PolicyCenter) permanece intacto, solo se agrega lógica de decisión en Fase 2.

**Nivel de complejidad:**
**MEDIO-ALTO** - Refactorización con Strategy Pattern requiere diseño cuidadoso, pero reutiliza componentes existentes. La modificación de `regex.csv` es crítica y requiere validación exhaustiva. Limpieza de código deprecado reduce riesgo de regresión.

**Riesgos técnicos conocidos:**
- **CRÍTICO**: Modificación de `regex.csv` (4000+ caracteres en 1 línea) puede romper validaciones existentes de riesgos si no se prueba exhaustivamente
- **ALTO**: Tests de regresión después de eliminar `TC_ADDITIONALINTEREST` deben cubrir TODOS los tipos de carga masiva
- **MEDIO**: Determinación automática Persona vs Compañía basada en apellidos puede causar errores si usuario ingresa nombre de compañía con apellido
- **BAJO**: Strategy Pattern agrega indirección que puede dificultar debugging inicial

**Patrones y convenciones del equipo:**
- **Gosu**: Código en inglés, comentarios en español, sin strings literales (usar constantes), indentación 2 espacios, sin punto y coma
- **Nomenclatura**: PascalCase para clases, camelCase para métodos/variables, UPPER_SNAKE_CASE para constantes
- **Enhancement**: Todas las extensiones nuevas fuera del paquete `sura` deben terminar en `_Ext`
- **Tests**: Directorio `gtest/`, extender de `EESuraTestBase`, usar `@RunLevel(NONE)`, patrón AAA obligatorio, `Mockito.spy()` para clase bajo prueba
- **Validaciones**: Comparar sobre constantes (evitar NPE), validar nulos antes de acceder propiedades
- **Archivos**: Sin líneas vacías al inicio/final, sin código comentado
...
**Framework**: GUnit (JUnit modificado de Guidewire) con Mockito para mocking | **Tipos**: Tests unitarios exclusivamente (Guidewire no permite tests de integración/E2E) | **Cobertura**: >70% (umbral mínimo Guidewire) | **Test Data Builders**: Reutilizar builders existentes en `EESuraTestBase` (`createGetPolicyLinePropertyAccessor`, `createGroupLifeLine`)

### Historias Relacionadas Consultadas

**Implementaciones similares analizadas:**
- Historia #915240 (Bug - Organizar campos errados detalle de cobro): Patrón de validación de integridad de datos en reportes masivos, manejo de archivos de errores
...
- **Factory Pattern**: Usado para selección de transformadores por tipo de archivo
- **DTO Pattern**: Usado en `CommonTransformer` para mapeo entre entidades y DTOs
- **mockStatic obligatorio**: Para enhancements y clases con métodos estáticos (`ContactUtil.createNewContactDummy()`)
- **Spy Pattern**: `Mockito.spy()` obligatorio para clase bajo prueba (evita problemas de ClassLoader)

**Mejores prácticas aplicadas:**
- Reutilización de `ContactUtil.createNewContactDummy()` en lugar de duplicar lógica
- Encapsulación de responsabilidades (Factory decide, Strategy ejecuta)
- Tests unitarios con patrón AAA, mocking apropiado de dependencias externas
- Validación de integridad en Fase 2 (PolicyCenter) en lugar de Fase 1 (Azure) - Separación de responsabilidades
- Limpieza de código deprecado antes de agregar nueva funcionalidad - Reduce deuda técnica

---

## Tareas de Implementación (Developer)

### Fase 0: Infraestructura y Migraciones
...
- [x] **Modificar validaciones de Azure Data Factory**
  ...
  - [ ] Ejecutar validación con dataset de prueba (50+ variaciones CSV: vacíos, parciales, completos)
  - [ ] Actualizar documentación de plantilla si es necesario

---

### Fase 1: Limpieza de Código Deprecado (Basado en Hito #2 del Arquitecto)

#### 📦 PolicyCenter - Limpieza Transformadores

- [x] **Eliminar referencia deprecada TC_ADDITIONALINTEREST** (AC: N/A - Limpieza técnica)
  - [x] Eliminar línea 9 en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/factories/TransformerFactoryProducer.gs`
  - [x] Modificar: `case typekey.MassiveLoadFileType.TC_ADDITIONALINTEREST.Code : return new BeneficiaryTransformerFactory()` → ELIMINAR caso completo

- [x] **Eliminar validación duplicada en OperationValidationFactory** (AC: N/A - Limpieza técnica)
  - [x] Eliminar líneas 20-22 en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/validations/factories/OperationValidationFactory.gs`
  - [x] Modificar: Condición especial `if(message.FileType == typekey.MassiveLoadFileType.TC_ADDITIONALINTEREST)` → ELIMINAR bloque completo

- [x] **Simplificar método getOperationValidation en EnrichmentProcessing** (AC: N/A - Limpieza técnica)
  - [x] Eliminar método `getOperationValidation()` completo (líneas 166-172) en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/batch/enrichment/EnrichmentProcessing.gs`
  - [x] Reemplazar llamada a `getOperationValidation()` por llamada directa a `getOperationValidationFactory().createOperationValidatorInstance()`
  - [ ] Test unitario: Verificar que validaciones de beneficiarios funcionan igual después del cambio

---

### Fase 2: Implementación de Strategy Pattern (Basado en Hitos #3-#6 del Arquitecto)

#### 📦 PolicyCenter - Interfaces y Contratos

- [x] **Crear interface ContactCreationStrategy** (AC: 1, 2, 3 - Base para decisión de creación)
  - [x] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/ContactCreationStrategy.gs`
  - [x] Definir método: `function createContact(requestDTO : MassiveUploadMessageDTO, number : String, commonTransformer : CommonTransformer) : Contact`
  - [x] Definir método: `function supportsType(documentType : String, documentNumber : String) : boolean`
  - [x] Documentar contrato con comentarios en español

#### 📦 PolicyCenter - Implementación de Estrategias

- [x] **Implementar DummyContactCreationStrategy** (AC: 1, 6 - Creación de contactos Dummy)
  - [x] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/DummyContactCreationStrategy.gs`
  - [x] Implementar `supportsType()`: retornar `true` si ambos campos vacíos
  - [x] Implementar `createContact()`:
    - Determinar tipo (Persona vs Compañía) basándose en presencia de apellidos en CSV
    - Llamar a `ContactUtil.createNewContactDummy()` reutilizando flujo uno-a-uno
    - Poblar datos desde CSV (nombre, apellidos si aplica)
    - Asignar dirección dummy estándar usando `ContactUtil.updateAddressDummy()`
  - [x] Test unitario en `gtest/sura/pc/massiveupload/strategy/DummyContactCreationStrategyTest.gs`:
    - Test: `testSupportsTypeMustReturnTrueWhenBothFieldsEmpty`
    - Test: `testSupportsTypeMustReturnFalseWhenAnyFieldHasValue`
    - Test: `testCreateContactMustReturnPersonDummyWhenLastNameExists`
    - Test: `testCreateContactMustReturnCompanyDummyWhenLastNameEmpty`
    - Test: `testCreateContactMustReuseContactUtilMethods`
    - Mock: `mockStatic(ContactUtil, {ContactUtil#createNewContactDummy(), ContactUtil#updateAddressDummy()})`

- [x] **Implementar CompleteContactCreationStrategy** (AC: 3 - Creación de contactos completos)
  - [x] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/CompleteContactCreationStrategy.gs`
  - [x] Implementar `supportsType()`: retornar `true` si ambos campos tienen valor
  - [x] Implementar `createContact()`:
    - Buscar contacto existente con `IContactPlugin.searchContact()`
    - Si no existe, retorna `null` (el `CommonTransformer` maneja el caso de crear nuevo)
    - Usar `CommonTransformer.contactToAdditionalInterest()` para mapeo a DTO
  - [x] Test unitario en `gtest/sura/pc/massiveupload/strategy/CompleteContactCreationStrategyTest.gs`:
    - Test: `testSupportsTypeMustReturnTrueWhenBothFieldsHaveValue`
    - Test: `testSupportsTypeMustReturnFalseWhenAnyFieldEmpty`
    - Test: `testCreateContactMustSearchExistingContact`
    - Test: `testCreateContactMustCreateNewWhenNotFound`
    - Mock: `_iContactPlugin`, `_commonTransformer`

#### 📦 PolicyCenter - Factory de Estrategias

- [x] **Implementar ContactCreationStrategyFactory** (AC: 1, 2, 3 - Decisión automática según tabla)
  - [x] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/ContactCreationStrategyFactory.gs`
  - [x] Implementar `getStrategy()`:
    - Validar tabla de decisión de 4 escenarios:
      - Vacío/Vacío → `DummyContactCreationStrategy`
      - Lleno/Lleno → `CompleteContactCreationStrategy`
      - Vacío/Lleno O Lleno/Vacío → `ValidationException` con mensaje claro
    - Retornar estrategia apropiada
  - [x] Declarar constantes para mensajes de error (sin strings literales)
  - [x] Test unitario en `gtest/sura/pc/massiveupload/strategy/ContactCreationStrategyFactoryTest.gs`:
    - Test: `testGetStrategyMustReturnDummyWhenBothFieldsEmpty`
    - Test: `testGetStrategyMustReturnCompleteWhenBothFieldsFilled`
    - Test: `testGetStrategyMustThrowExceptionWhenOnlyTypeEmpty`
    - Test: `testGetStrategyMustThrowExceptionWhenOnlyNumberEmpty`

---

### Fase 3: Refactorización de Transformadores (Basado en Hito #7 del Arquitecto)

#### 📦 PolicyCenter - Validaciones de Beneficiarios

- [x] **Modificar DefaultBeneficiaryValidation para soportar Dummy** (AC: 1, 2 - Campos opcionales)
  - [x] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/validations/factories/DefaultBeneficiaryValidation.gs`
  - [x] En `validatePrimaryBeneficiary()`: Remover `TIPO_DOCUMENTO_BENEFICIARIO_1` y `NUM_DOCUMENTO_BENEFICIARIO_1` de campos obligatorios
  - [x] En `validateNumberAndTypeDocumentBeneficiary()`: Permitir ambos campos vacíos (Dummy) o ambos llenos (Completo)
  - [x] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/validations/OperationValidationBase.gs`
  - [x] En `validateBeneficiaryDocument()`: Aplicar misma lógica de validación condicional
  - [x] Agregar display key: `Sura.MassiveUpload.Validations.beneficiaryDocumentIncomplete`
  - [x] Test unitario en `gtest/sura/pc/massiveupload/validations/factories/DefaultBeneficiaryValidationEESuraTestBase.gs`:
    - Test: `testValidateBeneficiaryDocumentWhenBothFieldsEmptyForDummy`
    - Test: `testValidateBeneficiaryDocumentWhenOnlyTypeFilledMustFail`
    - Test: `testValidateBeneficiaryDocumentWhenOnlyNumberFilledMustFail`

#### 📦 PolicyCenter - Refactorización Transformador de Beneficiarios

- [x] **Refactorizar DefaultBeneficiaryTransformer.createInterestContingent()** (AC: 1, 2, 3 - Integración de estrategias)
  - [x] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/DefaultBeneficiaryTransformer.gs`
  - [x] Simplificar método `createInterestContingent()` (líneas 85-95):
    - Eliminar búsqueda directa de contacto inline
    - Reemplazar con: `var strategy = ContactCreationStrategyFactory.getStrategy(documentType, documentNumber, _iContactPlugin)`
    - Crear contacto con: `var contact = strategy.createContact(requestDTO, number, _commonTransformer)`
    - Mantener lógica posterior intacta (mapeo a DTO, asignación de parentesco, etc.)
  - [x] Reducción esperada: ~30 líneas a ~5 líneas usando factory
  - [ ] Test unitario en `gtest/sura/pc/massiveupload/transformer/DefaultBeneficiaryTransformerTest.gs`:
    - Test: `testCreateInterestContingentMustUseDummyStrategyWhenFieldsEmpty`
    - Test: `testCreateInterestContingentMustUseCompleteStrategyWhenFieldsFilled`
    - Test: `testCreateInterestContingentMustPropagateValidationException`
    - Mock: `mockStatic(ContactCreationStrategyFactory, {ContactCreationStrategyFactory#getStrategy()})`
    - Spy: `_defaultBeneficiaryTransformer = Mockito.spy(new DefaultBeneficiaryTransformer(_commonTransformer, _iContactPlugin))`

---

### Fase 4: Extensión de CommonTransformer (Basado en Hito #8 del Arquitecto)

#### 📦 PolicyCenter - Soporte para Contactos Dummy

- [x] **Extender CommonTransformer.contactToAdditionalInterest()** (AC: 1, 6 - Mapeo de Dummy a DTO)
  - [x] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/common/CommonTransformer.gs`
  - [x] Agregar type checking al inicio del método `contactToAdditionalInterest()` (líneas 350-375):
    - Agregar: `if (contact typeis PersonDummy_Ext) { return setPersonDummyAdditionalInterestData(contact, requestDTO, number) }`
  - [x] Crear método `setPersonDummyAdditionalInterestData()`:
    - Mapear `PersonDummy_Ext` a `PersonExtDTO`
    - Poblar datos de contacto (nombre, apellidos, dirección)
    - Retornar `AdditionalInterestDTO` completo
  - [x] Test unitario en `gtest/sura/pc/massiveupload/transformer/common/CommonTransformerTest.gs`:
    - Test: `testContactToAdditionalInterestWhenIsPerson` (corregido para nuevo flujo)
    - Test: `testContactToAdditionalInterestWhenIsCompany` (corregido para nuevo flujo)

**Nota**: Solo se implementó soporte para `PersonDummy_Ext` ya que por regla de negocio los beneficiarios Dummy siempre son personas (tienen nombres y apellidos).

---

### Fase 5: Validación y Tests de Integración Manuales

#### 📦 Validación de Negocio

- [ ] **Crear dataset de prueba exhaustivo** (AC: 1, 2, 3, 4, 5)
  - [ ] Crear archivo CSV con 50+ escenarios:
    - 10 registros con campos vacíos (Dummy Persona)
    - 10 registros con campos completos (Contactos estándar)
    - 5 registros con campo tipo vacío + número lleno (Error Fase 2)
    - 5 registros con campo tipo lleno + número vacío (Error Fase 2)
    - 10 registros con reemplazo de beneficiarios existentes
    - 10 registros con asegurados sin beneficiarios previos
  - [ ] Ejecutar carga masiva en ambiente DEV
  - [ ] Validar ErrorFase1 y ErrorFase2 contienen errores esperados con mensajes claros
  - [ ] Validar beneficiarios Dummy creados correctamente en base de datos
  - [ ] Validar beneficiarios completos creados correctamente (regresión)
  - [ ] Validar reemplazo total de beneficiarios funciona (no fusión)

- [ ] **Validación de visualización en UI** (AC: 6)
  - [ ] Acceder a póliza master en PolicyCenter
  - [ ] Consultar riesgo con beneficiarios Dummy creados
  - [ ] Verificar que se muestra tipo "Dummy Persona"
  - [ ] Verificar que datos (nombre, apellidos) se visualizan correctamente
  - [ ] Verificar que beneficiarios estándar se siguen mostrando igual (regresión)
...
- [ ] **Ejecutar Agente Peer Review** (MANUAL)
  - Revisar cumplimiento de estándares de código Sura
  - Validar nomenclatura, formato, strings en constantes
  - Verificar tests unitarios con cobertura >70%
...
- [ ] **Crear Pull Request** (MANUAL)
  ...
  - Vincular a historia #825041

- [ ] **Ejecutar pipeline deployment DEV** (MANUAL)
  - Deployar cambios de Azure (regex.csv)
  - Deployar cambios de PolicyCenter (código Gosu)
  - Validar que no hay errores de compilación
  - Ejecutar suite de tests unitarios en pipeline

#### 📦 Testing Manual

- [ ] **Diseñar set de pruebas manuales** (MANUAL)
  - Crear casos de prueba para cada criterio de aceptación (1-6)
  - Definir datos de entrada y resultados esperados
  - Incluir pruebas de regresión (cargas de riesgos, beneficiarios estándar)

- [ ] **Ejecutar pruebas manuales** (MANUAL)
  - Ejecutar todos los casos de prueba en ambiente DEV
  - Documentar resultados (exitosos y fallidos)
...
- Fase 0: Habilita el flujo completo (sin este cambio, registros con campos vacíos nunca llegan a Fase 2)
- Fase 1: Limpieza técnica sin impacto directo en AC, pero reduce deuda técnica y riesgo de regresión
- Fase 2-4: Implementan AC 1, 2, 3 (creación automática según tabla de decisión)
- Fase 5: Valida AC 1, 2, 3, 4, 5, 6 (todos los escenarios de negocio)
- Fase 6: Asegura calidad y deployment correcto (no vinculado a AC específicos)
...
| 3 | Eliminar validación duplicada OperationValidationFactory | BAJO | 1.5h | 1.2h | 1.0h | 0.6h | 0.4h | **0.4h** |
| 4 | Simplificar método getOperationValidation | MEDIO | 4.3h | 2.7h | 1.7h | 1.7h | 1.1h | **0.7h** |
...
| 12 | Validación de visualización en UI | 1.1h | estimacion |
...
| 17 | Diseñar set de pruebas manuales | 2.0h | estimacion |
...
#### 📊 Estimación Completa: Tradicional vs Método Ceiba

**Conversión:** 1 punto = 1 día laboral = 9 horas

| Perfil | Desarrollo Tradicional (SIN IA) | Desarrollo con Método Ceiba (CON IA) | Ahorro de Tiempo |
|--------|----------------------------------|--------------------------------------|------------------|
...
| Perfil | Tareas de Código | Tareas Manuales | Total | Puntos |
|--------|------------------|-----------------|-------|--------|
| Junior | 63.3h | 15.8h | 79.1h | 8.8 puntos |
| Semi Senior | 42.1h | 15.8h | 57.9h | 6.4 puntos |
| **Senior** | **26.5h** | **15.8h** | **42.3h** | **4.7 puntos** |

##### Desarrollo con Método Ceiba (CON IA - Optimizado con herramientas de IA)

| Perfil | Tareas de Código (MC) | Tareas Manuales | Total | Puntos |
|--------|-----------------------|-----------------|-------|--------|
| Junior | 26.3h | 15.8h | 42.1h | 4.7 puntos |
| Semi Senior | 16.0h | 15.8h | 31.8h | 3.5 puntos |
| **Senior** | **10.5h** | **15.8h** | **26.3h** | **2.9 puntos** |

**Optimización en Tareas de Código:**
- Método Ceiba reduce el tiempo de desarrollo de código en **60%** aplicando descuento de IA sobre estimación tradicional
- Las tareas manuales (15.8h) no se benefician de IA y permanecen constantes en ambos escenarios

#### 🎯 Recomendación de Asignación para Sprint Planning

**Escenario SIN Método Ceiba (Tradicional):**
- Senior: **5 puntos**
- Semi Senior: **6-7 puntos**
- Junior: **9 puntos**

**Escenario CON Método Ceiba (Recomendado):**
- **Senior: 3 puntos** ⭐ (redondeado desde 2.9)
...
- Ambiente de desarrollo está configurado y funcional
- Acceso a Azure Data Factory para modificar regex.csv está disponible
- No hay cambios en requisitos durante la implementación

**Riesgos Identificados:**
- **CRÍTICO (4.3h de riesgo):** Modificación de regex.csv puede romper validaciones existentes - requiere dataset exhaustivo de prueba
- **ALTO (2h de riesgo):** Tests de regresión después de eliminar TC_ADDITIONALINTEREST deben cubrir todos los tipos de carga
- **MEDIO (1.5h de riesgo):** Determinación automática Persona vs Compañía puede causar errores si usuario ingresa nombre incorrecto

**Tareas con Alta Incertidumbre (Riesgo > 3h):**
- Tarea 1: Modificar validaciones de Azure Data Factory (Riesgo: 6h - CRÍTICO)
- Tarea 6: Implementar DummyContactCreationStrategy + Tests (Riesgo: 4h)

**Recomendaciones:**
- Considerar spike técnico para validación de regex.csv antes de implementación
- Ejecutar tests de regresión completos después de Fase 1 (limpieza código deprecado)
- Validar dataset de prueba con expedidores reales antes de deployment a producción
