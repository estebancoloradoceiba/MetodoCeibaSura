...
- [ ] **Modificar validaciones de Azure Data Factory**
  - [ ] Modificar archivo `VidaGrupoIAC/modules/datafactory/controlSchema/Beneficiaries/regex.csv`
  - [ ] Hacer opcional `TIPO_DOCUMENTO_BENEFICIARIO_X` agregando `|(^$)` al final de cada regex
  - [ ] Hacer opcional `NUM_DOCUMENTO_BENEFICIARIO_X` agregando `?` al cuantificador
...
- [ ] **Eliminar referencia deprecada TC_ADDITIONALINTEREST** (AC: N/A - Limpieza técnica)
  - [ ] Eliminar línea 9 en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/factories/TransformerFactoryProducer.gs`
  - [ ] Modificar: `case typekey.MassiveLoadFileType.TC_ADDITIONALINTEREST.Code : return new BeneficiaryTransformerFactory()` → ELIMINAR caso completo

- [ ] **Eliminar validación duplicada en OperationValidationFactory** (AC: N/A - Limpieza técnica)
  - [ ] Eliminar líneas 20-22 en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/validations/factories/OperationValidationFactory.gs`
  - [ ] Modificar: Condición especial `if(message.FileType == typekey.MassiveLoadFileType.TC_ADDITIONALINTEREST)` → ELIMINAR bloque completo

- [ ] **Simplificar método getOperationValidation en EnrichmentProcessing** (AC: N/A - Limpieza técnica)
  - [ ] Eliminar método `getOperationValidation()` completo (líneas 166-172) en `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/batch/enrichment/EnrichmentProcessing.gs`
  - [ ] Reemplazar llamada a `getOperationValidation()` por llamada directa a `getOperationValidationFactory().createOperationValidatorInstance()`
...
- [ ] **Crear interface ContactCreationStrategy** (AC: 1, 2, 3 - Base para decisión de creación)
  - [ ] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/ContactCreationStrategy.gs`
  - [ ] Definir método: `function createContact(requestDTO : MassiveUploadMessageDTO, number : String, commonTransformer : CommonTransformer) : Contact`
  - [ ] Definir método: `function supportsType(documentType : String, documentNumber : String) : boolean`
  - [ ] Documentar contrato con comentarios en español

#### 📦 PolicyCenter - Implementación de Estrategias

- [ ] **Implementar DummyContactCreationStrategy** (AC: 1, 6 - Creación de contactos Dummy)
  - [ ] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/DummyContactCreationStrategy.gs`
  - [ ] Implementar `supportsType()`: retornar `true` si ambos campos vacíos
  ...
  - [ ] Test unitario en `gtest/sura/pc/massiveupload/strategy/DummyContactCreationStrategyTest.gs`:
...
- [ ] **Implementar CompleteContactCreationStrategy** (AC: 3 - Creación de contactos completos)
  - [ ] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/CompleteContactCreationStrategy.gs`
  - [ ] Implementar `supportsType()`: retornar `true` si ambos campos tienen valor
  - [ ] Implementar `createContact()`:
    - Buscar contacto existente con `IContactPlugin.searchContact()`
    - Si no existe, crear nuevo contacto completo (comportamiento actual)
    - Usar `CommonTransformer.contactToAdditionalInterest()` para mapeo a DTO
  - [ ] Test unitario en `gtest/sura/pc/massiveupload/strategy/CompleteContactCreationStrategyTest.gs`:
...
- [ ] **Implementar ContactCreationStrategyFactory** (AC: 1, 2, 3 - Decisión automática según tabla)
  - [ ] Crear archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/strategy/ContactCreationStrategyFactory.gs`
  - [ ] Implementar `getStrategy()`:
  ...
  - [ ] Declarar constantes para mensajes de error (sin strings literales)
  - [ ] Test unitario en `gtest/sura/pc/massiveupload/strategy/ContactCreationStrategyFactoryTest.gs`:
...
- [ ] **Refactorizar DefaultBeneficiaryTransformer.createInterestContingent()** (AC: 1, 2, 3 - Integración de estrategias)
  - [ ] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/DefaultBeneficiaryTransformer.gs`
  - [ ] Simplificar método `createInterestContingent()` (líneas 85-95):
  ...
  - [ ] Reducción esperada: ~30 líneas a ~5 líneas usando factory
...
- [ ] **Extender CommonTransformer.contactToAdditionalInterest()** (AC: 1, 6 - Mapeo de Dummy a DTO)
  - [ ] Modificar archivo: `PolicyCenter/modules/configuration/gsrc/sura/pc/massiveupload/transformer/common/CommonTransformer.gs`
  - [ ] Agregar type checking al inicio del método `contactToAdditionalInterest()` (líneas 350-375):
    - Agregar: `if (contact typeis PersonDummy_Ext) { return setPersonDummyAdditionalInterestData(contact, requestDTO, number) }`
    - Agregar: `if (contact typeis CompanyDummy_Ext) { return setCompanyDummyAdditionalInterestData(contact, requestDTO, number) }`
  - [ ] Crear método `setPersonDummyAdditionalInterestData()`:
    - Mapear `PersonDummy_Ext` a `PersonExtDTO`
    - Asignar tipo de documento "DummyPerson" (typekey)
    - Poblar datos de contacto (nombre, apellidos)
    - Retornar `AdditionalInterestDTO` completo
  - [ ] Crear método `setCompanyDummyAdditionalInterestData()`:
    - Mapear `CompanyDummy_Ext` a `CompanyExtDTO`
    - Asignar tipo de documento "DummyCompany" (typekey)
    - Poblar datos de contacto (nombre compañía)
    - Retornar `AdditionalInterestDTO` completo
  - [ ] Test unitario en `gtest/sura/pc/massiveupload/transformer/common/CommonTransformerTest.gs`:
    - Test: `testContactToAdditionalInterestMustMapPersonDummy`
    - Test: `testContactToAdditionalInterestMustMapCompanyDummy`
    - Test: `testContactToAdditionalInterestMustPreserveStandardBehaviorForRegularContacts` (Regresión)
    - Mock: `_bundle.newBeanInstance(PersonDummy_Ext)`, `_bundle.newBeanInstance(CompanyDummy_Ext)`
...
- [ ] **Crear dataset de prueba exhaustivo** (AC: 1, 2, 3, 4, 5)
  - [ ] Crear archivo CSV con 50+ escenarios:
    - 10 registros con campos vacíos (Dummy Persona y Compañía)
...
- [ ] **Validación de visualización en UI** (AC: 6)
  ...
  - [ ] Verificar que se muestra tipo "Dummy Persona" o "Dummy Compañía"
